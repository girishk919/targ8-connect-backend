	...
	try {
		if (
			...
			return res
				.status(400)
				.json(
					'Only Individual Subscription must specify the company ID to which the subscription will be available.'
				);
		}
		...
		if (req.body.type === 'COMMON') {
			subscription = await Subscription.findOne({ title: req.body.title });
			if (subscription)
		...
		if (newSubscription) {
			const addActivityLog = new ActivityLog({
				person: req.user.id,
				role: req.user.role,
				heading: `${req.body.type} Subscription Added ${
...
const getSubscriptionById = async (req, res) => {
	let subscription = await Subscription.findById(req.params.id).populate(
		'features'
	);
	if (!subscription) return res.status(400).json('SUBSCRIPTION NOT FOUND');
	res.status(200).json({
...
const getAllSubscription = async (req, res) => {
	...
	if (req.user.role === 'ADMIN' || req.user.role == 'SUB_ADMIN') {
		if (req.query.company_id) {
			...
			subscription = await Subscription.find({ disabled: false }).populate(
				'features'
			);
		...
		if (company.planType === 'PYG' || company.plan.isExpired === true) {
			subscription = await Subscription.find({
				$or: [
					{ type: 'COMMON' },
			...
			var findSubscription = await Subscription.findOne({
...
const getAll = async (req, res) => {
	let subscription = await Subscription.find({ disabled: false }).populate(
		'features'
	);
...
const deleteSubscription = async (req, res) => {
	try {
		...
		const addActivityLog = new ActivityLog({
			...
			message: 'Deleted a subscription: ' + subscription.title + '.',
		});
